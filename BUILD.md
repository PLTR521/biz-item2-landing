...
| 2026-07-17 | **리브랜딩: 제품명·URL을 Email Deliverability로 변경**(사용자 지시). Vercel 프로젝트 biz-item2-landing → email-deliverability 리네이밍(Claude in Chrome) → 새 URL email-deliverability.vercel.app, 구 URL도 계속 서빙. 랜딩 표기 전면 교체(타이틀/OG/Nav/Footer/파이프라인/FAQ). API CORS를 신·구 origin allowlist로 확장(SendGuard-AI 커밋 208dfa1, `LANDING_ORIGIN` env 콤마 구분 지원). GitHub 리포명은 유지 | 9b86a4e |
